...
def customer_add_order(request):
    if request.method == "POST":
        ...
        if len(order_details) > 0:
            ...
            # Step 1 - Creaet a charge: this will charge customer's card
...
def driver_pick_order(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return JsonResponse({"status": "failed", "error": "You need new token"})
...
def driver_get_revenue(request):
    ...
    revenue = {}
    today = timezone.now()
    current_weekdays = [today + timedelta(days = i) \
